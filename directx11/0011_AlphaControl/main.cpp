...
#define WINDOW_CLASS    L"アルファ値をコントロール [L:濃・J:薄]"
...
HRESULT InitBuffer()
{
    ...
    // シェーダへのバッファ転送には最低16バイトが必要。
    // アルファ値を格納するfloat型単体だと32bitの4バイトしかないのでシェーダへは送れない。
    // サイズ指定には直に16と書いておくか、
    // ConstantBuffer構造体でfloat m_alphaの他にダミーを持たせる形でXMVECTOR paddingみたいにサイズを満たしてやる方法とがある。
    // だが、XMVECTORのサイズは32バイトでfloatと合わせて36バイトなので余計なサイズ指定になる。
    // だから、アルファ値のようにfloat単体をシェーダに渡すときは、
...
HRESULT InitTexture()
{
    ...
    // テクスチャの読み込み
    if (FAILED(D3DX11CreateShaderResourceViewFromFile(g_device, L"leaf.png", NULL, NULL, &g_shaderResourceView, NULL)))
    {
...
VOID OnUpdate()
{
    ...
    if (g_alpha < 0.25f) g_alpha = 0.25f;         // 下限0.25f
