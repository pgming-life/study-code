...
#define WINDOW_CLASS    L"文字列の描画"
#define WINDOW_TITLE    WINDOW_CLASS
#define WINDOW_WIDTH    750
#define WINDOW_HEIGHT   500

// フォント情報
static const FLOAT g_fontSize = 50.f;               // フォントサイズ 50px
static const std::vector<std::string> stringList    // 文字列リスト
{
    "あいうえお",
    "山田太郎",
    "ABCDEFGHIJKLMN",
...
// シェーダデータ初期化リスト
static const ShaderData initShaderData[COLOR_NUMMAX] =
{
    {NONE,      "VSMain",   "PSMain"},
    {RED,       "VSMain",   "PSRed"},
    {LIME,      "VSMain",   "PSLime"},
    {AQUA,      "VSMain",   "PSAqua"},
    {YELLOW,    "VSMain",   "PSYellow"},
    {PURPLE,    "VSMain",   "PSPurple"},
    {ORANGE,    "VSMain",   "PSOrange"},
};
...
ComPtr<ID3D11PixelShader> g_pixelShader[COLOR_NUMMAX];      // ピクセルシェーダ
...
std::list<FontText*> g_fontText;    // フォントテキスト
std::list<BOOL> g_fontFlag;         // 半角全角フラグ
std::list<BOOL> g_lineFlag;         // 行フラグ
...
HRESULT InitShader()
{
    ...
    ComPtr<ID3DBlob> vertexShader, pixelShader[COLOR_NUMMAX];

    // 頂点シェーダをロードしコンパイル
    if (FAILED(D3DX11CompileFromFile(L"shaders.hlsl", NULL, NULL, initShaderData[0].vsEntry, "vs_4_0", NULL, NULL, NULL, vertexShader.GetAddressOf(), NULL, NULL)))
    {
...
HRESULT InitTexture()
{
    ...
    // 半角全角判定
    for (size_t i = 0; i < stringList.size(); i++)              // 行抽出
    {
        g_lineFlag.push_back(TRUE);

        for (size_t j = 0; j < stringList[i].length(); j++)     // 列抽出
        {
            if (IsDBCSLeadByte(stringList[i][j]) == 0)
            {
                // 半角の場合
                g_fontFlag.push_back(FALSE);
            }
            else
            {
                // 全角の場合
                g_fontFlag.push_back(TRUE);
                j++;
            }
            g_lineFlag.push_back(FALSE);
        }
    }

    // 要素数を確保
    for (auto it = g_fontFlag.begin(); it != g_fontFlag.end(); ++it)
        g_fontText.push_back(&FontText());

    // フォントテキスト初期化
    USHORT k = 0;       // 文字バイトカウント
    USHORT n = 0;       // 遅延カウント
    INT p = -1;         // 行カウント
    std::list<BOOL>::iterator itLineFlag = g_lineFlag.begin();
    std::list<BOOL>::iterator itFontFlag = g_fontFlag.begin();
    std::list<FontText*>::iterator itFontText = g_fontText.begin();
    for (; itFontText != g_fontText.end(); ++itFontText)
    {
        (*itFontText) = new FontText();
        ...
        // １文字を抽出しフォントのシェーダリソースビューを生成
        TCHAR b[32];
        CString c = stringList[p].substr(k += n, *itFontFlag ? 2 : 1).c_str();
        lstrcpy(b, c);
        (*itFontText)->InitChar(b);
        n = *itFontFlag ? 2 : 1;
...
VOID OnRender()
{
    ...
    // フォントテキストを描画
    USHORT i = 0;
    FLOAT offsetLine = -g_fontSize;     // 行オフセット
    FLOAT offsetRow = -g_fontSize;      // 列オフセット
    ConstantBuffer cBuffer;
    std::list<BOOL>::iterator itLineFlag = g_lineFlag.begin();
    for (auto itFontText = g_fontText.begin(); itFontText != g_fontText.end(); ++itFontText)
    {
        // 行判定
        if (*itLineFlag)
        {
            // フォントカラーの変更
            g_context->PSSetShader(g_pixelShader[i].Get(), nullptr, 0);
            i++;

            // 次の行
            offsetLine += g_fontSize;

            // リセット
            offsetRow = -g_fontSize;

            ++itLineFlag;
        }

        // 変数代入
        g_world = XMMatrixTranslation(offsetRow += g_fontSize, offsetLine, 0.f);
        ...
        // テクスチャをシェーダに登録
        g_context->PSSetShaderResources(0, 1, (*itFontText)->GetAddressSRV());
