...
#define WINDOW_CLASS    L"マルチテクスチャ"
...
// ピクセルシェーダエントリ名
static const LPCSTR psEntry[PS_NUMMAX] =
{
    "psMain",
    "psMulti",
};
...
ID3D11PixelShader* g_pixelShader[PS_NUMMAX];        // ピクセルシェーダ
...
HRESULT InitShader()
{
    ID3DBlob* vertexShader, * pixelShader[PS_NUMMAX];
    ...
    // ピクセルシェーダをロードしコンパイル
    for (UINT i = 0; i < PS_NUMMAX; i++)
    {
        if (FAILED(D3DX11CompileFromFile(L"shaders.hlsl", NULL, NULL, psEntry[i], "ps_4_0", NULL, NULL, NULL, &pixelShader[i], NULL, NULL)))
        {
...
HRESULT InitTexture()
{
    ...
    // テクスチャの読み込み
    if (FAILED(D3DX11CreateShaderResourceViewFromFile(g_device, L"wall.jpg", NULL, NULL, &g_shaderResourceView1, NULL)))
    {
        MessageBox(NULL, L"テクスチャ1を読み込めませんでした。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
        return E_FAIL;
    }
    if (FAILED(D3DX11CreateShaderResourceViewFromFile(g_device, L"moss.jpg", NULL, NULL, &g_shaderResourceView2, NULL)))
    {
...
{
    // ワールドマトリックスの初期化
    g_world1 = XMMatrixTranslation(-0.6f, 0.0f, 0.0f);
    g_world2 = XMMatrixTranslation(0.6f, 0.0f, 0.0f);
    g_world3 = XMMatrixIdentity();      // 引数なしで全ての軸で0.0f(移動なし)
}
...
VOID OnRender()
{
    ...
    g_context->PSSetShader(g_pixelShader[PS_MAIN], nullptr, 0);
    ...
    // ピクセルシェーダを変更
    g_context->PSSetShader(g_pixelShader[PS_MULTI], nullptr, 0);

    // テクスチャ1とテクスチャ2をシェーダに登録し合成
    g_context->PSSetShaderResources(0, 1, &g_shaderResourceView1);
    g_context->PSSetShaderResources(1, 1, &g_shaderResourceView2);

    // 変数3代入
    cBuffer.m_world = XMMatrixTranspose(g_world3);
...
VOID OnDestroy()
{
    ...
    for (UINT i = 0; i < PS_NUMMAX; i++) SAFE_RELEASE(g_pixelShader[i]);
