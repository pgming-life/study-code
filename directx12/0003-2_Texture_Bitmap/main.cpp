...
#define WINDOW_CLASS	L"テクスチャ(ビットマップ)"
...
HRESULT InitBuffer()
{
	...
	// テクスチャを作成
	{
		// ビットマップファイルの読み込み
		HBITMAP	hBitmap;
		hBitmap = (HBITMAP)LoadImage(0, L"texture.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
		if (!hBitmap)
		{
			MessageBox(NULL, L"テクスチャを読み込めませんでした。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
			return E_FAIL;
		}
		// DDB＆GetPixelでピクセルデータを読み込む簡易版なのでアルファ値(不透明度)には対応していない。
		...
		textureDesc.Width = bmp.bmWidth;
		textureDesc.Height = bmp.bmHeight;
		...
		// コミットリソースを作成
		if (FAILED(g_device->CreateCommittedResource(&heapProperties, D3D12_HEAP_FLAG_NONE, &textureDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(g_texture.GetAddressOf()))))
		{
			MessageBox(NULL, L"テクスチャ用コミットリソースを作成できませんでした。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
			return E_FAIL;
		}

		// テクスチャー用のディスクリプタヒープの設定
		...
		// テクスチャディスクリプタヒープの作成
		if (FAILED(g_device->CreateDescriptorHeap(&descriptorHeapDesc, IID_PPV_ARGS(g_srvHeap.GetAddressOf()))))
		{
			MessageBox(NULL, L"テクスチャディスクリプタヒープを作成できませんでした。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
			return E_FAIL;
		}
		...
		// CPUディスクリプタハンドルの設定
		D3D12_CPU_DESCRIPTOR_HANDLE cdHandle = {};
		cdHandle = g_srvHeap->GetCPUDescriptorHandleForHeapStart();

		// シェーダリソースビューを作成
		g_device->CreateShaderResourceView(g_texture.Get(), &srvDesc, cdHandle);

		// 画像データをサブリソースへコピー
		D3D12_BOX box = { 0, 0, 0, (UINT)bmp.bmWidth, (UINT)bmp.bmHeight, 1 };
		if (FAILED(g_texture->WriteToSubresource(0, &box, &pixel[0], sizeof(uint32_t) * bmp.bmWidth, sizeof(uint32_t) * bmp.bmWidth * bmp.bmHeight)))
		{
			MessageBox(NULL, L"画像データのサブリソースへのコピーに失敗しました。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
			return E_FAIL;
		}
	}
