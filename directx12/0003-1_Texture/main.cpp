...
// テクスチャ情報
static const UINT g_textureWidth = 256;
static const UINT g_textureHeight = 256;
static const UINT g_texturePixelSize = 4;	// テクスチャ内のピクセルを表すために使用されるバイト数
...
// テクスチャデータジェネレータ
static const std::vector<UINT8> GenerateTextureData();
...
HRESULT InitBuffer()
{
	...
	// テクスチャを作成
	{
		...
		// コミットリソースを作成
		if (FAILED(g_device->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE, &textureDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(g_texture.GetAddressOf()))))
		{
		...
		{
			MessageBox(NULL, L"アップロードバッファを作成できませんでした。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
			return E_FAIL;
		}

		// テクスチャデータ取得
		const std::vector<UINT8> texture = GenerateTextureData();
		// データを中間アップロードヒープにコピーしてから、アップロードヒープからテクスチャリソースへのコピーをスケジュールする。

		// テクスチャデータの設定
		D3D12_SUBRESOURCE_DATA textureData = {};
		textureData.pData = &texture[0];
		textureData.RowPitch = g_textureWidth * g_texturePixelSize;
...
static const std::vector<UINT8> GenerateTextureData()
{
	const UINT rowPitch = g_textureWidth * g_texturePixelSize;
	const UINT cellPitch = rowPitch >> 3;		    // チェッカーボードテクスチャのセルの幅
	const UINT cellHeight = g_textureWidth >> 3;	// チェッカーボードテクスチャのセルの高さ
	const UINT textureSize = rowPitch * g_textureHeight;

	std::vector<UINT8> data(textureSize);
	UINT8* pData = &data[0];

	// シンプルな黒と白のチェッカーボードテクスチャを生成
	for (UINT n = 0; n < textureSize; n += g_texturePixelSize)
	{
		UINT x = n % rowPitch;
		UINT y = n / rowPitch;
		UINT i = x / cellPitch;
