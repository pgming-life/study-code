...
#define WINDOW_CLASS	L"テクスチャ(WIC)"
...
HRESULT InitBuffer()
{
	...
	// ComなのでCoInitializeExとCoUninitializeで挟む。
	...
	// テクスチャを作成
	{
		...
		if (FAILED(LoadWICTextureFromFile(g_device.Get(), L"texture.jpg", &g_texture, wicData, subresouceData)))
		{
			MessageBox(NULL, L"テクスチャを読み込めませんでした。", WINDOW_TITLE, MB_OK | MB_ICONERROR);
			return E_FAIL;
		}
		// BMP,JPEG,PNG,TIFF,GIFを読み込める。
		...
		// テクスチャデータ更新
		UpdateSubresources(g_commandList.Get(), g_texture.Get(), textureUploadHeap.Get(), 0, 0, 1, &subresouceData);
